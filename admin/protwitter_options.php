...
<div class="wrap">
	<?php    echo "<h2>" . __( 'Pro Twitter Options', 'protwitter_trdom' ) . "</h2>"; ?>
	<form name="protwitter_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		...
		<?php    echo "<h4>" . __( 'Pro Twitter Options Settings', 'protwitter_trdom' ) . "</h4>"; ?>
		
		<p><?php _e("Your Twitter ID: " ); ?><input type="text" name="protwitter_twitter_id" value="<?php echo $twitter_id; ?>" size="20"><?php _e(" ex: twitter" ); ?></p>
		
			
		<p class="submit">
		<input type="submit" name="Submit" value="<?php _e('Update Options', 'protwitter_trdom' ) ?>" />
		</p>
	</form>
